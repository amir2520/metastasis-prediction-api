...
class MetricComparerConfig:
    _target_: str = "metastatic.model_selection.model_selector.MetricComparer"
...
class BinaryF1ScoreMetricComparerConfig(MetricComparerConfig):
    ...
    metric_name: str = "crossval_f1_mean"


@dataclass
class ModelSelectorConfig:
    _target_: str = "metastatic.model_selection.model_selector.ModelSelector"
    mlflow_run_id: Optional[str] = MISSING              # SI("${infrastructure.mlflow.run_id}")
...
@dataclass
class MetastaticDetectionModelSelectorConfig(ModelSelectorConfig):
    must_be_better_metric_comparers: dict[str, MetricComparerConfig] = field(
        default_factory=lambda: {
            "crossval_f1_mean": BinaryF1ScoreMetricComparerConfig(),
        }
    )
